...
<body>
  ...
  <div class='title_line'>브랜디/꼬냑 상품 목록</div>
  
  <aside class="aside_left">
  <c:forEach var="categoryVO" items="${list_2 }">
    <c:set var="categoryGrpID" value="${categoryVO.grpID}" />
    <c:set var="categoryName" value="${categoryVO.categoryName}" />
    <c:if test="${categoryGrpID eq 2}">
  ...
  <aside class="aside_right">
    ...
    <span class='menu_divide' >│</span>    
    <a href="./list_all_2.do?now_page=${param.now_page == null ? 1 : param.now_page}&word=${param.word }">목록형</a>  
  </aside>
  
 <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_all_2_grid.do'>
      <input type='text' name='word' id='word' value='${param.word }' style='width: 20%;'>
      <button class="press-button" type='submit'>검색</button>
      <c:if test="${param.word.length() > 0 }">
        <button class="press-button" type='button' 
                     onclick="location.href='./list_all_2_grid.do?&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
  ...
  <div style='width: 100%;'> <%-- 갤러리 Layout 시작 --%>
    <c:forEach var="productsVO" items="${list }" varStatus="status">
      ...
      <div onclick="location.href='./read.do?productID=${productID}&word=${param.word }&now_page=${param.now_page == null ? 1 : param.now_page }&categoryID=${param.categoryID }'" 
             style='width: 19%; height: 200px; float: left; margin: 0.5%; padding: 0.5%; background-color: #EEEFFF; text-align: center; cursor: pointer;'>
