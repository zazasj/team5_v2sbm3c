...
<div class='title_line'>배송 ID: ${shippingVO.shippingID }번 수정</div>
...
<form name='frm' method='post' action='/shipping/update.do' enctype="multipart/form-data">
  
  <input type='file' name='file1MF' id='file1MF'  placeholder="파일 선택"style="width: 15%"><br>
  <input type='hidden' name='shippingID' value='${shippingVO.shippingID }'>
  <div style="text-align: center;">
      <label>배송 상태</label>
<select id="deliveryStatus" name="deliveryStatus" size="1" style="width: 7%">
    <option value="주문완료" ${shippingVO.deliveryStatus == '주문완료' ? 'selected' : ''}>주문완료</option>
    <option value="배송중" ${shippingVO.deliveryStatus == '배송중' ? 'selected' : ''}>배송중</option>
    <option value="배송완료" ${shippingVO.deliveryStatus == '배송완료' ? 'selected' : ''}>배송완료</option>
    <option value="반품" ${shippingVO.deliveryStatus == '반품' ? 'selected' : ''}>반품</option>
    <option value="취소" ${shippingVO.deliveryStatus == '취소' ? 'selected' : ''}>취소</option>
</select>

    
      <label>배송 타입</label>
<select id="shippingType" name="shippingType" size="1" style="width: 10%">
    <option value="우체국택배" ${shippingVO.shippingType == '우체국택배' ? 'selected' : ''}>우체국택배</option>
    <option value="롯데택배" ${shippingVO.shippingType == '롯데택배' ? 'selected' : ''}>롯데택배</option>
    <option value="로젠택배" ${shippingVO.shippingType == '로젠택배' ? 'selected' : ''}>로젠택배</option>
    <option value="한진택배" ${shippingVO.shippingType == '한진택배' ? 'selected' : ''}>한진택배</option>
    <option value="cj대한통운택배" ${shippingVO.shippingType == 'cj대한통운택배' ? 'selected' : ''}>cj대한통운택배</option>
    <option value="직접입력" ${shippingVO.shippingType == '직접입력' ? 'selected' : ''}>직접입력</option>
</select>
