...
public class SupplierProc implements SupplierProcInter{
    ...
    public ArrayList<SupplierVO> list_all(){
	  ...
	  public ArrayList<SupplierVO> list_by_supplierid_search(HashMap<String, Object> hashMap) {
	    ArrayList<SupplierVO> list = this.supplierDAO.list_by_supplierid_search(hashMap);
	    return list;
	  }

	  @Override
	  public int search_count(HashMap<String, Object> hashMap) {
	    int cnt = this.supplierDAO.search_count(hashMap);
	    return cnt;
	  }
	  ...
	  public ArrayList<SupplierVO> list_by_supplierid_search_paging(SupplierVO supplierVO) {
	    ...
	    int begin_of_page = (supplierVO.getNow_page() - 1) * Suppliers.RECORD_PER_PAGE;
	    ...
	    // 2 페이지 = 10 + 10: 20
	    // 3 페이지 = 20 + 10: 30
	    int end_num = begin_of_page + Suppliers.RECORD_PER_PAGE;   
	    
	    supplierVO.setStart_num(start_num);
	    supplierVO.setEnd_num(end_num);
	    
	    ArrayList<SupplierVO> list = this.supplierDAO.list_by_supplierid_search_paging(supplierVO);
	  ...
	  public String pagingBox(int now_page, String word, String list_file, int search_count) {
	    ...
	    int total_page = (int)(Math.ceil((double)search_count / Suppliers.RECORD_PER_PAGE));
	    // 전체 그룹  수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
	    int total_grp = (int)(Math.ceil((double)total_page / Suppliers.PAGE_PER_BLOCK)); 
	    // 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
	    int now_grp = (int)(Math.ceil((double)now_page / Suppliers.PAGE_PER_BLOCK));  
	    
	    // 1 group: 1, 2, 3 ... 9, 10
	    // 2 group: 11, 12 ... 19, 20
	    // 3 group: 21, 22 ... 29, 30
	    int start_page = ((now_grp - 1) * Suppliers.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작  페이지  
	    int end_page = (now_grp * Suppliers.PAGE_PER_BLOCK);               // 특정 그룹의 마지막 페이지   
...
//	    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	    ...
	    int _now_page = (now_grp - 1) * Suppliers.PAGE_PER_BLOCK;  
	    ...
	    _now_page = (now_grp * Suppliers.PAGE_PER_BLOCK)+1; //  최대 페이지수 + 1 
	  ...
	  public int update_text(SupplierVO supplierVO) {
	    int cnt = this.supplierDAO.update_text(supplierVO);
	    return cnt;
	  }
	  
	  @Override
	    public int update_file(SupplierVO supplierVO) {
	        int cnt = this.supplierDAO.update_file(supplierVO);
	        return cnt;
	    }
}
