...
public class Order_payProc implements Order_payProcInter {
  ...
  public ArrayList<Order_payVO> list_by_memberno_search(HashMap<String, Object> hashMap) {
    ArrayList<Order_payVO> list = this.order_payDAO.list_by_memberno_search(hashMap);
    return list;
  }

  @Override
  public int search_count(HashMap<String, Object> hashMap) {
    int cnt = this.order_payDAO.search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<Order_payVO> list_by_memberno_search_paging(Order_payVO order_payVO) {
    ...
    int begin_of_page = (order_payVO.getNow_page() - 1) * Order_pay.RECORD_PER_PAGE;
   
    int start_num = begin_of_page + 1;
    
    int end_num = begin_of_page + Order_pay.RECORD_PER_PAGE;   

    order_payVO.setStart_num(start_num);
    order_payVO.setEnd_num(end_num);
    
    ArrayList<Order_payVO> list = this.order_payDAO.list_by_memberno_search_paging(order_payVO);
  ...
  public String pagingBox(int memberno, int now_page, String word, String list_file, int search_count){
    ...
    int total_page = (int)(Math.ceil((double)search_count / Order_pay.RECORD_PER_PAGE));
    // 전체 그룹  수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
    int total_grp = (int)(Math.ceil((double)total_page / Order_pay.PAGE_PER_BLOCK)); 
    // 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
    int now_grp = (int)(Math.ceil((double)now_page / Order_pay.PAGE_PER_BLOCK));  
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
      str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&memberno="+memberno+"'>이전</A></span>"); 
    } 
    ...
    for(int i=start_page; i<=end_page; i++){ 
      ...
      if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='"+list_file+"?word="+word+"&now_page="+i+"&memberno="+memberno+"'>"+i+"</A></span>");   
      } 
    } 
    ...
    if (now_grp < total_grp){ 
      str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&memberno="+memberno+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
